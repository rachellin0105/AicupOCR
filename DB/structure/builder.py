...
class Builder(Configurable):
    ...
    @property
    def model_name(self):
        return self.model + '-' + getattr(DB.structure.model, self.model).model_name(self.model_args)

    def build(self, device, distributed=False, local_rank: int = 0):
        Model = getattr(DB.structure.model,self.model)
